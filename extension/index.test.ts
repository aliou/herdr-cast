...
describe("herdr hook", () => {
  ...
  describe("error block lifecycle", () => {
    ...
    it("clears the error block on a retry agent_start", () => {
      const mock = createMockPi();
      herdr(mock.pi);

      // Fresh user turn starts the run.
      mock.fire("agent_start");
      // Low-level run errors (e.g. a recoverable provider error); pi retries.
      mock.pi.events.emit(AD_NOTIFY_DONE_EVENT, { status: "error" });
      mock.emit.mockClear();
    ...
    it("unblocks when a retry recovers (done ok)", () => {
      ...
      mock.fire("agent_start");
      mock.pi.events.emit(AD_NOTIFY_DONE_EVENT, { status: "error" });
      // Retry's agent_start clears the prior error; a later successful done
      // is then a no-op because the block is already clear.
      mock.emit.mockClear();
      mock.fire("agent_start");
